...
- **判断に必要な最小限で止める**。結論が変わらない検証はやらない
  - 例: 「A と B のどちらを先にやるか」を決めたいだけなら、A が実現可能で B が重いと分かれば十分。B の全項目を実測で潰す必要はない
- 深掘りする前に、**結論を数行チャットに出して粒度を確認する**。先回りして全部調べない
  - 意思決定に必要な情報は箇条書き 3-5 行で足りることが多い
  - 網羅的な検証結果・表は「もっと詳しく」と言われてから出す
- Issue / PR に書く粒度もこれに合わせる。判断の材料と結論だけ書き、調査の全量は載せない
  - 例: 「images 移行に必要なこと: angle の追加実装 / CloudFront のオリジン変更。qa 検証も必要で簡単には移行できない」→ この 3 行で意思決定できるならそれ以上書かない
- 網羅性を示すために表や HTML を作らない。表・図・コード片が意思決定に必要なときだけ作る
